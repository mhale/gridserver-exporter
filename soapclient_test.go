...
func TestNewSOAPClient(t *testing.T) {
	...
	tests := []struct {
		...
		{"FullPathWithTimeout",
			...
			&SOAPClient{"http://director:1234/livecluster/webservices", "user", "pass", &tls.Config{InsecureSkipVerify: true}, 10 * time.Second},
			false,
		},
		{"SecureFullPathSkipVerify",
			args{"https://user:pass@director:1234/livecluster/webservices", false, 5 * time.Second},
			&SOAPClient{"https://director:1234/livecluster/webservices", "user", "pass", &tls.Config{InsecureSkipVerify: true}, 5 * time.Second},
			false,
		},
		{"SecureFullPathWithVerify",
			args{"https://user:pass@director:1234/livecluster/webservices", true, 5 * time.Second},
			&SOAPClient{"https://director:1234/livecluster/webservices", "user", "pass", &tls.Config{InsecureSkipVerify: false}, 5 * time.Second},
			false,
		},
		{"NoScheme",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewSOAPClient(tt.args.uri, tt.args.sslVerify, tt.args.timeout)
